...
void letterbox(const cv::Mat &image, cv::Mat &padded_image, letterbox_t &pads, const float scale, const cv::Size &target_size, const cv::Scalar &pad_color)
{
    // 调整图像大小
    ...
    pads.x_pad = pad_width / 2;
    pads.y_pad = pad_height / 2;
    pads.scale = scale;

    // 在图像周围添加填充
    cv::copyMakeBorder(resized_image, padded_image, pads.y_pad, pad_height - pads.y_pad, pads.x_pad, pad_width - pads.x_pad, cv::BORDER_CONSTANT, pad_color);
}
