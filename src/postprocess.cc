...
#include <vector>
// 标签文件路径（相对于可执行文件的目录）
// 注意：如果模型文件不存在，会回退到默认值
#ifdef RKNN_YOLOV8_LABEL_PATH
#define LABEL_NALE_TXT_PATH RKNN_YOLOV8_LABEL_PATH
#else
...
static int loadLabelName(const char *locationFilename, char *label[])
{
    ...
    // 检查文件是否存在
    FILE *fp = fopen(locationFilename, "r");
    if (fp == NULL)
    {
        printf("Warning: Label file not found at %s\n", locationFilename);

        // 尝试备用路径（大约回溯到项目根目录）
        const char *backupPath = "../../model/coco_80_labels_list.txt";
        printf("Trying backup path: %s\n", backupPath);

        fp = fopen(backupPath, "r");
        if (fp == NULL)
        {
            printf("Error: Label file not found at any path. Detection results will show 'null'.\n");
            printf("Please ensure coco_80_labels_list.txt is in ./model/ or ../../model/ directory.\n");
            return -1;
        }
        fclose(fp);
        readLines(backupPath, label, OBJ_CLASS_NUM);
    }
    else
    {
        fclose(fp);
